...
def classify_day_mood(day_messages, blocks):
    ...
    if avg_neg >= 1.5 or len(argument_blocks) >= 2:
        return 'explosive'
    if avg_neg >= 0.8 or len(argument_blocks) >= 1:
        return 'heated'
    if avg_neg >= 0.3:
        return 'tense'
    if total_positive > len(day_messages) * 0.1:
        return 'loving'
    return 'neutral'
...
def load_all_data():
    ...
    with open(SIGNAL_DESKTOP_JSON, encoding='utf-8') as f:
...
def generate_month_file(month_str, month_days, all_days):
    ...
    lines.append("**Legend**: → = You sent | ← = She sent | 📞 = Phone call | 🔒 = Signal call\n")
    ...
    for summary in daily_summaries:
        ...
        for i, msg in enumerate(all_msgs):
            ...
            body = msg.get('body', '')
            source_tag = " `[SMS]`" if msg.get('source') == 'sms' else ""
...
def generate_week_file(month_str, week_key, week_summaries, month_days):
    ...
    lines.append(f"**Contact**: {CONTACT_NAME} ({CONTACT_PHONE})")
    lines.append("**Legend**: → = You sent | ← = She sent | 📞 = Phone call")
    lines.append("**This is a WORD-FOR-WORD log** — every message is exactly as sent.\n")
...
def main():
    ...
    index_lines.append("**Total Messages**: ~202,500")
    index_lines.append("**Date Range**: October 2024 – February 2026")
